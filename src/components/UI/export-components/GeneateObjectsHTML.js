export const GenerateObjectsHTML = (objects) => {
    ...
    const generateBorderStyles = (obj) => {
        if (obj.type !== "figure" && obj.type !== "group")
            return "";
        const borderProps = [
            {
                side: "top",
        ...
        const definedBorders = borderProps.filter((b) => b.width && b.style && b.color);
        if (definedBorders.length === 4 &&
            definedBorders.every((b) => b.width === definedBorders[0].width &&
        ...
        return definedBorders
            .map((b) => `border-${b.side}: ${toPx(b.width)} ${b.style} ${toRGB(b.color)};`)
            .join("");
    };
    const generateStyles = (obj, isChild = false, isText = false, isOuter = true, isGroupChild = false) => {
        const styles = [];
        if (isOuter) {
            if (!isChild) {
                styles.push(`position: absolute`);
                if (obj.x !== undefined)
                    styles.push(`left: ${toPx(obj.x)}`);
                if (obj.y !== undefined)
                    styles.push(`top: ${toPx(obj.y)}`);
                if (obj.zIndex !== undefined)
                    styles.push(`z-index: ${obj.zIndex}`);
            }
            ...
            if (obj.type === "group") {
                if (obj.display)
                    styles.push(`display: ${obj.display}`);
                ...
                if (obj.alignItems)
                    styles.push(`align-items: ${obj.alignItems}`);
                if (obj.gap)
                    styles.push(`gap: ${toPx(obj.gap)}`);
            ...
            if (obj.fontSize)
                styles.push(`font-size: ${toPx(obj.fontSize)}`);
            if (obj.fontFamily)
                styles.push(`font-family: ${obj.fontFamily}`);
            if (obj.fontWeight)
            ...
            if (obj.color)
                styles.push(`color: ${toRGB(obj.color)}`);
            if (obj.maxLines) {
            ...
            if (obj.objectFit)
                styles.push(`object-fit: ${obj.objectFit}`);
            if (obj.type === "image" || obj.type === "figure") {
        ...
        if (obj.paddingTop)
            styles.push(`padding-top: ${toPx(obj.paddingTop)}`);
    ...
    const generateObjectHTML = (obj, isChild = false, isGroupChild = false) => {
        const outerStyles = generateStyles(obj, isChild, obj.type === "text", true, isGroupChild);
        const innerStyles = generateStyles(obj, isChild, obj.type === "text", false, isGroupChild);
        const conditionAttr = obj.condition
            ? `data-condition='${JSON.stringify(obj.condition)}'`
            : "";
        const dynamicAttr = obj.type === "image" &&
        ...
        if (obj.type === "text") {
            ...
            const maxLines = typeof rawMaxLines === "number" && rawMaxLines > 0 ? rawMaxLines : 20;
            let lines = String(obj.content || "").split("\n");
            const isTruncated = lines.length > maxLines;
            lines = lines.slice(0, maxLines);
            const processedContent = lines.map((line) => escapeHTML(line)).join("<br>") +
                (isTruncated ? "<br>" : "");
            const content = `<div ${isChild ? idAttr : ""} ${conditionAttr} class="text-field ${isChild ? "banner-object-child" : ""}" data-max-lines="${obj.maxLines || ""}" style="${innerStyles};">${processedContent}</div>`;
            return isChild
                ? content
        ...
        if (obj.type === "image") {
            if (!obj.src)
                return "";
            const content = `<img ${isChild ? idAttr : ""} ${conditionAttr} ${dynamicAttr} src="${obj.src}" alt="${obj.name || "image"}" loading="eager" class="image-field ${isChild ? "banner-object-child" : ""}" style="${innerStyles}; background: transparent;" />`;
            return isChild
                ? content
                : `<div ${idAttr} ${conditionAttr} ${classAttr} style="${outerStyles}">${content}</div>`;
        }
        if (obj.type === "figure") {
            const content = `<div ${isChild ? idAttr : ""} ${conditionAttr} class="banner-figure ${isChild ? "banner-object-child" : ""}" style="${innerStyles}"></div>`;
            return isChild
                ? content
                : `<div ${idAttr} ${conditionAttr} ${classAttr} style="${outerStyles}">${content}</div>`;
        }
    ...
    const generateTopLevelHTML = (obj) => {
        const objectHTML = generateObjectHTML(obj, false, false);
        if (obj.conditionForAbstract) {
            const abstractConditionAttr = `abstract-data-condition='${JSON.stringify(obj.conditionForAbstract)}'`;
