...
export const ExportToHTML_3 = (objects: BannerObject[]): string => {
  ...
  const generateStyles = (
    ...
    // Позиционирование (исключаем left, top, width, height для дочерних текстовых элементов)
    if (!isChild || !isText) {
    ...
    if (!excludeBorders) {
      ...
      if (
        ...
        borders.push({
          side: "border-right",
          style: `${toPx(obj.borderRightWidth)} ${obj.borderRightStyle} ${toRGB(
            obj.borderRightColor
          )}`,
        });
      }
  ...
  const generateObjectHTML = (
    ...
    const styles = generateStyles(obj, isChild, obj.type === "text");
    ...
    const classAttr = isChild
      ...
      : `class="banner-object"`;
    let positionStyles = styles;
    if (!isChild) {
      positionStyles = `position: absolute; ${styles}`;
      if (obj.type === "image" || obj.type === "figure") {
        positionStyles += "; cursor: move";
      }
    }
    ...
    if (obj.type === "text") {
      ...
      }" style="${styles}; display: block">${obj.content || ""}</div>`;
      return isChild
        ? content
        : `<div ${idAttr} ${conditionAttr} ${classAttr} style="${positionStyles}">${content}</div>`;
    } else if (obj.type === "image") {
      const content = `<img ${isChild ? idAttr : ""} ${conditionAttr} src="${
        obj.src || ""
      }" alt="image" class="image-field ${
        isChild ? "banner-object-child" : ""
      }" style="${isChild ? styles : "width: 100%; height: 100%"}" />`;
      return isChild
        ? content
        : `<div ${idAttr} ${conditionAttr} ${classAttr} style="${positionStyles}">${content}</div>`;
    } else if (obj.type === "figure") {
      const outerStyles = generateStyles(obj, isChild, false, true);
      const outerPositionStyles = isChild
        ? outerStyles
        : `position: absolute; ${outerStyles}; cursor: move`;
      const content = `<div ${
        isChild ? idAttr : ""
      ...
      return isChild
        ? content
        : `<div ${idAttr} ${conditionAttr} ${classAttr} style="${outerPositionStyles}">${content}</div>`;
      ...
      const groupContent = obj.children
        ...
        .join("");
      return `<div ${idAttr} ${conditionAttr} ${classAttr} style="${positionStyles}">${groupContent}</div>`;
    }
  ...
  // ... остальной код ExportToHTML_3 (без изменений) ...
